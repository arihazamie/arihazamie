...
function Row({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex justify-between items-center py-2 border-b border-[#1a1a1a] last:border-0 text-sm">
      ...
      <span className={accent ? "text-emerald-400 font-medium" : "text-neutral-300"}>{value}</span>
    </div>
  );
}
      ...
      <div className="max-w-2xl mx-auto px-6 pt-20 pb-28">
        ...
        <div className="mb-14">
          <div className="flex items-center gap-2 mb-5">
            ...
            <span className="text-sm text-neutral-500">Finance & Accounting Expertise</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-4 leading-snug">
            Paham konsepnya,<br />tahu cara kerjanya.
          </h1>
          <p className="text-neutral-400 text-sm leading-relaxed">
            Penjelasan Finance & Accounting, perpajakan Indonesia, Excel, dan Accurate Desktop —
            lengkap dengan contoh nyata penggunaan di industri.
          </p>
        </div>
        ...
        ══════════════════════════════════════════════════ */}
        <section>
          <h2 className="text-xl font-semibold text-white mb-1">Finance & Accounting</h2>
          ...
          {/* Debit & Kredit */}
          <div className="mb-10">
            <h3 className="text-base font-semibold text-white mb-1">Debit & Kredit</h3>
            <p className="text-neutral-400 text-sm leading-relaxed mb-4">
              Setiap transaksi dicatat di dua sisi:{" "}
              <span className="text-white">Debit</span> dan{" "}
              <span className="text-white">Kredit</span> dengan jumlah yang selalu sama.
              Artinya bukan "masuk" atau "keluar" — tapi bergantung pada jenis akun.
            </p>
            ...
            <div className="border border-[#1f1f1f] rounded-xl overflow-hidden mb-5">
              ...
              ] as [string, string, string, string][]).map(([type, dr, cr, ex], i) => (
                <div
                  key={type}
                  className={`grid grid-cols-4 px-4 py-3 text-sm border-b border-[#1a1a1a] last:border-0 ${i % 2 === 0 ? "bg-[#0d0d0d]" : ""}`}
                >
                  <span className="text-neutral-300 font-medium">{type}</span>
                  <span className={`text-center font-mono text-xs ${dr.includes("Naik") ? "text-sky-400" : "text-neutral-600"}`}>{dr}</span>
                  <span className={`text-center font-mono text-xs ${cr.includes("Naik") ? "text-emerald-400" : "text-neutral-600"}`}>{cr}</span>
                  <span className="text-right text-neutral-500 text-xs">{ex}</span>
                </div>
              ))}
            </div>

            <p className="text-neutral-500 text-xs mb-3">Contoh pencatatan nyata:</p>
            <div className="space-y-3">
              {([
                {
                  tx: "Terima pembayaran klien Rp 5.000.000",
                  dr: "Bank",             drAmt: "5.000.000",
                  cr: "Pendapatan Jasa",  crAmt: "5.000.000",
                  note: "Bank (aset) naik → Debit. Pendapatan naik → Kredit.",
                },
                {
                  tx: "Bayar gaji karyawan Rp 3.500.000",
                  dr: "Beban Gaji", drAmt: "3.500.000",
                  cr: "Bank",       crAmt: "3.500.000",
                  note: "Beban naik → Debit. Bank (aset) turun → Kredit.",
                },
                {
                  tx: "Beli perlengkapan tunai Rp 800.000",
                  dr: "Beban Operasional", drAmt: "800.000",
                  cr: "Kas",              crAmt: "800.000",
                  note: "Beban naik → Debit. Kas (aset) turun → Kredit.",
                },
              ] as {tx:string;dr:string;drAmt:string;cr:string;crAmt:string;note:string}[]).map((j) => (
                <div key={j.tx} className="bg-[#0d0d0d] border border-[#1f1f1f] rounded-xl px-5 py-4">
                  <p className="text-neutral-500 text-xs mb-3">{j.tx}</p>
                  <div className="font-mono text-xs space-y-1.5 mb-3">
                    <div className="flex justify-between">
                      <span><span className="text-sky-400 mr-3">Dr</span><span className="text-neutral-200">{j.dr}</span></span>
                      <span className="text-neutral-200">Rp {j.drAmt}</span>
                    </div>
                    <div className="flex justify-between pl-5">
                      <span><span className="text-emerald-400 mr-3">Cr</span><span className="text-neutral-400">{j.cr}</span></span>
                      <span className="text-neutral-400">Rp {j.crAmt}</span>
                    </div>
                  </div>
                  <p className="text-neutral-600 text-xs pt-3 border-t border-[#1a1a1a]">{j.note}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Laporan Keuangan */}
          <div className="mb-6">
            <h3 className="text-base font-semibold text-white mb-1">3 Laporan Keuangan Utama</h3>
            ...
            <div className="space-y-4">
              ...
              ] as {name:string;en:string;desc:string;formula:string}[]).map((r) => (
                <div key={r.name} className="bg-[#0d0d0d] border border-[#1f1f1f] rounded-xl px-5 py-4">
                  <div className="flex items-baseline gap-2 mb-2">
                    <span className="font-semibold text-white text-sm">{r.name}</span>
                    <span className="text-neutral-600 text-xs">{r.en}</span>
                  </div>
                  <p className="text-neutral-400 text-sm leading-relaxed mb-1">{r.desc}</p>
        ...
        ══════════════════════════════════════════════════ */}
        <section>
          <h2 className="text-xl font-semibold text-white mb-1">Perpajakan Indonesia</h2>
          <p className="text-neutral-500 text-sm mb-8">
            ...
            {([
              {
                ...
                example: "Gaji Rp 8 jt/bulan → hitung PKP → potong PPh 21 → karyawan terima take-home pay.",
                formula: "PKP = Gaji Setahun − Biaya Jabatan (maks Rp6 jt) − PTKP\nPPh 21 = PKP × tarif progresif (5% – 35%)",
              ...
              },
              {
                ...
                example: "Bayar desainer Rp 5 jt → potong PPh 23 2% (Rp 100rb) → transfer ke desainer Rp 4,9 jt → setor Rp 100rb ke DJP.",
                formula: "PPh 23 = Bruto × 2%   (jasa/sewa)\nPPh 23 = Bruto × 15%  (dividen / bunga / royalti)",
                deadline: ["Setor: tgl 10", "Lapor SPT Masa: tgl 20"],
              },
              {
                code: "PPN 11%",
                ...
                example: "Jual produk DPP Rp 10 jt → tagih klien Rp 11,1 jt (inc. PPN) → terbitkan Faktur Pajak → setor PPN ke DJP.",
                formula: "PPN = DPP × 11%\nKurang/Lebih Bayar = PPN Keluaran − PPN Masukan",
                deadline: ["Faktur Pajak: akhir bulan", "Lapor SPT Masa PPN: akhir bulan +1"],
              },
            ] as {code:string;full:string;when:string;who:string;example:string;formula:string;deadline:string[]}[]).map((tax) => (
              <div key={tax.code}>
                <div className="flex items-baseline gap-2 mb-1 flex-wrap">
                  <h3 className="text-base font-semibold text-white">{tax.code}</h3>
                  <span className="text-neutral-500 text-xs">— {tax.full}</span>
                </div>
                <div className="flex gap-2 mb-4 flex-wrap">
                  {tax.deadline.map(d => <Chip key={d}>{d}</Chip>)}
                </div>
                <div className="space-y-2 text-sm text-neutral-400 leading-relaxed mb-3">
                  <p>
                    <span className="text-neutral-300 font-medium">Kapan muncul? </span>
                    {tax.when}
                  </p>
                  <p>
                    <span className="text-neutral-300 font-medium">Siapa menanggung? </span>
                    {tax.who}
                  </p>
                  <p>
                    <span className="text-neutral-300 font-medium">Contoh: </span>
                    {tax.example}
                  </p>
                </div>
        ...
        ══════════════════════════════════════════════════ */}
        <section>
          <h2 className="text-xl font-semibold text-white mb-1">Microsoft Excel</h2>
          <p className="text-neutral-500 text-sm mb-8">
          ...
          <div className="space-y-8">
            {([
              ...
              },
              {
                ...
                example: "Row: Departemen | Column: Bulan | Value: Sum(Jumlah)\n→ 5.000 baris transaksi → laporan rekapitulasi siap dalam 30 detik.",
              },
              {
                fn: "SUMIF / SUMIFS",
                syntax: "=SUMIFS(range_jumlah, range_kriteria1, kriteria1, ...)",
              ...
              },
              {
                ...
                example: '=IF(D2 > BatasAnggaran, "⚠ OVER BUDGET", "OK")\n→ Monitoring budget bulanan otomatis per baris transaksi.',
              },
              {
                fn: "INDEX + MATCH",
                syntax: "=INDEX(kolom_hasil, MATCH(nilai_cari, kolom_cari, 0))",
            ...
            ] as {fn:string;syntax:string;what:string;when:string;example:string}[]).map((fn) => (
              <div key={fn.fn}>
                <div className="flex items-baseline gap-2 mb-3 flex-wrap">
                  <h3 className="text-base font-semibold text-white">{fn.fn}</h3>
                  <code className="font-mono text-xs text-neutral-500">{fn.syntax}</code>
                </div>
                <div className="space-y-2 text-sm text-neutral-400 leading-relaxed mb-2">
                  <p>
                    <span className="text-neutral-300 font-medium">Fungsi: </span>{fn.what}
                  </p>
                  <p>
                    <span className="text-neutral-300 font-medium">Dipakai saat: </span>{fn.when}
                  </p>
                </div>
        ...
        ══════════════════════════════════════════════════ */}
        <section>
          <h2 className="text-xl font-semibold text-white mb-1">Accurate Desktop</h2>
          <p className="text-neutral-500 text-sm mb-2">
            ERP akuntansi terpopuler di Indonesia — dipakai lebih dari 300.000 perusahaan, dari UMKM hingga menengah.
          </p>
          <p className="text-neutral-400 text-sm leading-relaxed mb-8">
            Accurate mengintegrasikan seluruh siklus keuangan dalam satu sistem: pembelian, penjualan,
            kas, pajak, hingga laporan keuangan — semua terhubung dan otomatis membuat jurnal.
          </p>
          ...
          <div className="space-y-7">
            {([
              {
                ...
                tags: ["Company Setup", "Chart of Accounts", "Opening Balance", "Periode Fiskal"],
              },
              {
                step: "02",
                ...
                tags: ["Purchase Order", "Goods Receipt", "Invoice Vendor", "Hutang AP"],
              },
              {
                step: "03",
                ...
                tags: ["Sales Order", "Invoice Penjualan", "Faktur Pajak", "Piutang AR"],
              },
              {
                step: "04",
                ...
                tags: ["Jurnal Umum", "Buku Besar", "Rekonsiliasi Bank", "Petty Cash"],
              },
              {
                step: "05",
                ...
                tags: ["PPh 21 Payroll", "PPh 23 Pemotongan", "PPN e-Faktur", "e-SPT DJP"],
              },
              {
                step: "06",
            ...
            ] as {step:string;title:string;desc:string;why:string;tags:string[]}[]).map((m) => (
              <div key={m.step} className="flex gap-5">
                <span className="text-emerald-500/50 font-mono text-xs font-bold mt-0.5 w-6 flex-shrink-0">{m.step}</span>
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-white mb-1">{m.title}</h3>
                  <p className="text-neutral-400 text-sm leading-relaxed mb-2">{m.desc}</p>
                  <p className="text-neutral-600 text-xs leading-relaxed mb-3">
                    <span className="text-neutral-500">Mengapa penting: </span>{m.why}
                  </p>
                  <div>
        ...
        {/* ── Experience note ── */}
        <section>
          <h2 className="text-xl font-semibold text-white mb-4">Pengalaman Lapangan</h2>
          <p className="text-neutral-400 text-sm leading-relaxed">
            Seluruh pemahaman di atas didukung pengalaman nyata sebagai{" "}
            <span className="text-white font-medium">Bendahara Desa Sungai Jering</span> —
            mengelola dana Rp 100–250 juta/triwulan menggunakan{" "}
            <span className="text-white font-medium">Siskeudes</span>:
            input APBDes, pencatatan BKU (setara buku kas/bank), pengelolaan SPP/SPM,
            hingga penyusunan LRA & SPJ sebagai pertanggungjawaban ke pemerintah daerah.
          </p>
          <div className="mt-4">
            <Tag>Siskeudes</Tag>
            <Tag>APBDes</Tag>
